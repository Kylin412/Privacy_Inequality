...
def intersection_matrix(matrices):
    ...
    n = len(matrices[0])
    return [
        [int(all(m[i][j] == 1 for m in matrices)) for j in range(n)]
        for i in range(n)
    ]


# === 4. 汇总所有实验，按 row 求交与众数 ===
def compute_intersection_and_consensus(file_pattern, output_csv):
    """
    ...
    for row in all_rows:
        ...
        mat_result = intersection_matrix(row_matrices)
...
if __name__ == "__main__":
    ...
    compute_intersection_and_consensus("output-loop1-exp*.jsonl", "loop1_intersection.csv")