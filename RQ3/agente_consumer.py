...
from configs.config_consumer_agente import API_KEY, API_BASE_URL, MODEL_NAME, TEMPERATURE, EXPERT_PROMPT_TEMPLATE
...
def build_input_and_output_from_row(row: Dict[str, Any]) -> Tuple[str, str]:
    # Construct raw input data (only retain decision rationales)
    input_data = f"""
[Share Decision Rationale]
{row.get('share_reason')}

[Final Purchase/Exit Rationale]
{row.get('final_reason')}
    ...
    try:
        ...
        print(f"⚠️ Failed to parse vector/matrix (consumer_id={row.get('consumer_id')}): {e}")
...
def run_expert_labeling(input_csv: str, output_csv: str):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_index):
            ...
            row_display = idx + 1
            try:
                row_idx, label, consumer_id, err_trace, raw_reply = future.result()
                assert row_idx == idx
                ...
                if err_trace:
                    print(f"⚠️ Error processing row {row_display} (consumer_id={consumer_id}), default label 0 assigned")
                    print("------ traceback ------")
                    print(err_trace)
                else:
                    preview = (raw_reply[:40] + "...") if isinstance(raw_reply, str) and len(raw_reply) > 40 else raw_reply
                    print(f"   [{done_count}/{total}] Row={row_display}, consumer_id={consumer_id}, Label={label}, RawReply={preview!r}")
